...
<div class="flex">
    ...
    <div class="px-5 w-10/12">
        <div class="mt-7 font-bold text-3xl">Edit Kategori</div>
        @if (count($errors) > 0)
      ...
      @endif
        ...
        @endif
        <form action="{{route('dashboard.kategori_put', $data['id'])}}" method="post" class="mt-5" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="id_admin" value="{{Auth::user()->id}}">
            <div class="flex flex-col w-1/2">
                <div class="mt-2 w-full space-y-2">
                    <div class="">Nama Kategori</div>
                    <input type="text" name="nama" value="{{old('nama', $data['nama'])}}" class="border border-black px-3 py-2 w-full" placeholder="Nama Kategori">
                </div>
                <button>
